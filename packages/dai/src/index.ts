...
export interface PatternModule {
  ...
  stat: "NIL" | "ADD" | "MOD" | "DEL";
  patd: {
    panm: string; padf: "V" | "R"; patp: "STG" | "LIN"; pasp: "CON" | "SCL";
    pami: number; pama: number; pacl: number; parw: number; pahl: number; pavl: number; pbxc: number; pbxr: number;
  };
...
export interface SymbolModule {
  ...
  stat: "NIL" | "ADD" | "MOD" | "DEL";
  symd: {
    synm: string; sydf: "V" | "R";
    sycl: number; syrw: number; syhl: number; syvl: number; sbxc: number; sbxr: number;
  };
  sxpo: string[];
...
export interface LinestyleModule {
  ...
  lind: {
    linm: string;
    licl: number; lirw: number; lihl: number; livl: number; lbxc: number; lbxr: number;
  };
  lxpo: string[];
...
export function parse(text: string): DaiFile {
  ...
  const out: DaiFile = { lbid: null, colours: [], lookups: [], patterns: [], symbols: [], linestyles: [], unknown: [] };
...
function parseUSList(payload: string): string[] {
  return (payload.endsWith(UnitSep) ? payload.slice(0, -1) : payload).split(UnitSep);
}
 ...
 */
function parseLine<Definition extends { [key: string]: number }>(definition: Definition, raw: string) {
  let start = 0;
  return Object.fromEntries(Object.entries(definition).map(([key, len]) => {
    let value: string;
    if (len === Variable) {
...
function parseColourTable(fields: FieldRaw[]): ColourTable {
  ...
  const { rcid, stat, ctus } = parseLine({
    modn: 2, rcid: 5, stat: 3, ctus: Variable
  }, cols.raw);
  ...
  return { rcid: parseInt(rcid, 10), stat: stat as ColourTable["stat"], ctus, entries };
}
...
function parseLookup(fields: FieldRaw[]): LookupEntry {
  ...
  const { rcid, stat, obcl, ftyp, dpri, rpri, tnam } = parseLine({
    modn: 2, rcid: 5, stat: 3, obcl: 6, ftyp: 1, dpri: 5, rpri: 1, tnam: Variable
  }, lu.raw);
...
function parsePattern(fields: FieldRaw[]): PatternModule {
  ...
  const pxpo = fields.filter(f => f.tag === "PXPO").map(f => f.raw.replace(/\x1F+$/, ""));
  const pcrf = parseColourRef(fields.find(f => f.tag === "PCRF")?.raw ?? "");
  const pbtmRows = fields.filter(f => f.tag === "PBTM").map(f => f.raw.replace(/\x1F+$/, ""));
  const pvctRows = fields.filter(f => f.tag === "PVCT").map(f => f.raw.replace(/\x1F+$/, ""));

  const mod: PatternModule = { rcid: parseInt(rcid, 10), stat: stat as PatternModule["stat"], patd, pxpo, pcrf: pcrf || {}, };
  if (pbtmRows.length) mod.pbtm = pbtmRows; else if (pvctRows.length) mod.pvct = pvctRows;
  return mod;
}
...
function parseSymbol(fields: FieldRaw[]): SymbolModule {
  ...
  const sxpo = fields.filter(f => f.tag === "SXPO").map(f => f.raw.replace(/\x1F+$/, ""));
  const scrf = parseColourRef(fields.find(f => f.tag === "SCRF")?.raw ?? "");
  const sbtm = fields.filter(f => f.tag === "SBTM").map(f => f.raw.replace(/\x1F+$/, ""));
  const svct = fields.filter(f => f.tag === "SVCT").map(f => f.raw.replace(/\x1F+$/, ""));
  ...
  if (sbtm.length) sym.sbtm = sbtm; else if (svct.length) sym.svct = svct;
  return sym;
}
...
function parseLinestyle(fields: FieldRaw[]): LinestyleModule {
  ...
  const lxpo = fields.filter(f => f.tag === "LXPO").map(f => f.raw.replace(/\x1F+$/, ""));
  const lcrf = parseColourRef(fields.find(f => f.tag === "LCRF")?.raw ?? "");
  const lvct = fields.filter(f => f.tag === "LVCT").map(f => f.raw.replace(/\x1F+$/, ""));
