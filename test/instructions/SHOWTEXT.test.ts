...
describe('TX', () => {
  ...
  test("TX(OBJNAM,1,2,3,'15110',0,0,CHBLK,26)", () => {
    const style = instructionsToStyles(`TX(OBJNAM,1,2,3,'15110',0,0,CHBLK,26)`)[0] as SymbolLayerSpecification
  ...
  describe('text-size', () => {
    [
      ...
      test(`CHARS=${chars} => ${expected}`, () => {
        const style = instructionsToStyles(`TX('Hello',1,1,1,'${chars}')`)[0] as SymbolLayerSpecification
  ...
  describe('text-anchor', () => {
    [
      ...
      test(`HJUST=${hjust} VJUST=${vjust} => ${expected}`, () => {
        const style = instructionsToStyles(`TX('Hello',${hjust},${vjust})`)[0] as SymbolLayerSpecification
...
describe('TE', () => {
  test(`TE('Nr %s','OBJNAM',3,1,2,'15110',1,0,CHBLK,29)`, () => {
    const style = instructionsToStyles(`TE('Nr %s','OBJNAM',3,1,2,'15110',1,0,CHBLK,29)`)[0] as SymbolLayerSpecification
    expect(style.layout?.["text-field"]).toEqual(['format', 'Nr ', ['get', 'OBJNAM']])
  })

  test(`TE('%03.0lf deg','ORIENT',1,1,2,'15110',0,-1,CHBLK,11)`, () => {
    const style = instructionsToStyles(`TE('%03.0lf deg','ORIENT',1,1,2,'15110',0,-1,CHBLK,11)`)[0] as SymbolLayerSpecification
...
describe('formatToExpression', () => {
  const examples: [string, string, any][] = [
    ["%s", "OBJNAM", ['get', 'OBJNAM']],
    ["Hello %s World", "OBJNAM", ['format', 'Hello ', ['get', 'OBJNAM'], ' World']],
    ["clr cl %4.1lf", "VRTCLR", ['format', 'clr cl ', ['number-format', ['get', 'VRTCLR'], { 'min-fraction-digits': 1, 'max-fraction-digits': 1 }]]],
    ['%03.0lf deg', 'ORIENT', ['format', ['number-format', ['get', 'ORIENT'], { 'min-fraction-digits': 0, 'max-fraction-digits': 0 }], ' deg']],
