...
export function parse(input: string): Instruction[] {
  return input.split(';').filter(i => i.trim() !== '').map(instruction => {
    const match = instruction.trim().match(/^([A-Z]{2})\((.*?)\)?$/);
    if (!match || !match[1] || !match[2]) {
      throw new Error(`Invalid command format: ${JSON.stringify(instruction)}`);
    }
    const [, cmd, params] = match;
    const paramList = params?.split(',').map(p => p.trim()).map(param => {
